...
| Primitive / capability | What you can build with it | Upper bound vs Devin out-of-the-box | Class |
|---|---|---|---|
| Sessions + self-hosted tools | Unattended multi-file coding: 12/13 graded runs 4/4 on a 27-file migration, 0 nudges in all 10 completed runs, $41–62, 2–6.5 min (B) | No durable notion of a *job*: budget stop is a mid-edit guillotine; all failures collapse into `retries_exhausted` | A |
| Event history + usage | Full forensic reconstruction (version→span→tool→cost) from 1–2 API calls; exact cost reconciliation across threads ($0.04 == 0.02+0.01+0.01) (I) | Forensics, not operations: no alerting, no stuck-session signal, no changed-files record | A |
| Compaction | 5.4 MB / 60-turn Opus run: 2 compactions, 867,646→40,753 tokens (95.3%), ~1.3 K-token cost each, exact early-constraint recall (A) | Model-dependent: Haiku never compacts and dies at 200 K (`prompt is too long`); the event payload is `{event_id, at}` — everything else is inference | A/C |
| Prompt caching | 93% cache-read fraction; $4.6/active-minute on an 868 K context (I) | Rosters degrade it (each child is a fresh prefix — that's where the 3–4× subagent tax goes) | A |
| Memory Store | Durable, versioned, attributable storage: 260 writes/12 workers/4.7 s zero failures; CAS via `expected_content_sha256` (1 winner, 5×409); full provenance + redaction (E) | **Storage, not retrieval**: nothing is injected; retrieval is the model choosing to `rg` a mount under an Anthropic-authored prompt you can only append to | A store / C retrieval |
| Cross-session learning | Interpreter-mismatch replay: with memory 1 install attempt / 45 s / $12 vs without 3 attempts / 69 s / $17 (K-13) | Pays only when rediscovery is expensive; on a cheap fact the write-back round cost 2.7× the cold baseline (E-15) | C |
| Skills | Versioned playbooks the agent follows verbatim once told where they live (K-12) | Attachment alone is a **silent no-op** (3/3 sessions denied the playbook existed); no listing/loading tool; no agent-side publishing | C |
| Built-in subagents | ≥7 concurrent children, per-thread cost, tool grants enforced at the tool layer, version pinning, evidence-based synthesis of conflicting reports (F) | Depth 1 (enforced twice); no cancel/timeout/heartbeat from inside; oversize replies (~533 KB) silently vanish 3/3; concurrent edits are silent last-writer-wins; no quality gain measured, 3–4× cost | A |
| Steering | `user.interrupt` accepted in 0.5 s; genuine re-planning with correct progress audit (vs 95 s waiting out a tool call without it) (K-1/2) | No "queue my message" primitive; interrupt-then-message is a client pattern you must implement | A/B |
| Ask-and-block | Custom tool parks a session indefinitely; answered at 900 s and 4501 s, resumed in 0.2 s; $18 for 945 s wall (20.3 s active) — parking is ~free (K-4/5) | `requires_action` is overloaded (machine-wait and human-wait look identical); disambiguation requires event correlation | B |
| CI / PR loop | One session took PR #6 red→green: read diff/comments/checks, fixed, tested locally, pushed, polled to `success`, replied inline — 10 MCP calls, 91 s active, $58 (K-9) | The mechanics are at parity; the missing piece is purely the **trigger** (no event ingress, §4.3) | A |
| Deployments | Cron (min floor) with full session capability (subagents, MCP, memory, outcome rubric); 8/8 fires, median 5.9 s jitter; ~61 s ticket-detection by polling Linear (H, K) | Stateless: every fire is a **new cold session**; no concurrency control (3 overlapping observed), no retry, no catch-up; one bad fire auto-pauses the whole deployment | A trigger / D lifecycle |
| Outcome evaluation | `user.define_outcome` rubric graded a run `needs_revision`, correctly catching `CPU count:` vs required `cpus:` (H-17) | Slow (28 consecutive `ongoing` spans, >13 min under load); used by 0 of 164 sessions observed — success is otherwise not a native field | A |
| Agent-as-code | 100% code-expressible; drift == [] round-trips; 409 optimistic concurrency; byte-exact roll-forward rollback; pinning fails closed (404 on bad version); 12 agents in 4.8 s (D) | No aliases/channels ("prod" can't be a pointer); no version delete/tag/prune; no server-side desired state — convergence is client code; no reverse index version→sessions | A/B |
| Canarying | Three pinned versions over one benchmark, zero orchestration, per-variant cost free from `session.usage` (D-12) | No deterministic replay: identical pin+prompt gave 164 vs 248 output tokens (+51%); no seed/temperature on the resource | A |
| Observability | SSE median 9 ms; fleet rollup 164 sessions / $104.42 in 2 calls / 0.96 s; `retry_status` (`retrying`/`terminal`/`exhausted`) is the recoverable-vs-fatal discriminator (I) | History and webhooks are disjoint surfaces (only 5 of 35+44 types overlap); budget/requires_action/idled are webhook-only; no aggregate/error-rate/alerting API | A/C |
| Budgets | Hard spend ceilings enforced at event admission (turn 12 rejected after 11 ran) (I-9) | A guillotine, not a brake: single turn overshot a $0.05 cap to $0.11 (2.2×); no wrap-up turn; stop reason says nothing about whether the work finished (B-9) | A |
| Model choice | Per-agent-version model pinning; per-roster-entry models (incl. one advisor) | **No routing** — see §4.5 | A config / D routing |
