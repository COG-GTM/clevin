...
|---|---|---|
| **Autonomous execution** | Hosted loop that plans, edits and runs commands unattended for hours; self-compacts. 12/13 graded runs 4/4 on a 27-file migration, 0 nudges, $41–62. | A turn, not a job: stopping ≠ finishing (budget killed a run mid-edit), all failures collapse to `retries_exhausted`, history/volume/memory commit independently, and no verification primitive — testing is just bash. |
| **Session management** | Durable addressable sessions: resume with history, roster, sandbox and git HEAD intact (0.2 s after a 75-min park); fleet-wide listing. | Frozen and un-branchable: only `tools`/`mcp_servers` are mutable mid-session, and fork/checkpoint/clone all 404 — you cannot try two approaches from one state. |
| **Memory & learning** | Durable, versioned, attributable store mounted into the sandbox with CAS writes: 260 writes/12 workers/4.7 s; repeat task 69 s/$17 → 45 s/$12. | Storage, not retrieval: nothing is injected, the model must `rg` a mount, so store size is free until read; memories are redact-only. Curation (Dreams) exists but is access-gated and untested. |
| **Subagents / delegation** | Rostered child agents in parallel isolated threads with per-thread cost, enforced tool grants and version pinning; ≥7 concurrent. | Depth 1, roster-only (nothing spawns an unrostered agent); no cancel/timeout/heartbeat; ~533 KB replies vanish silently; 3–4× cost for no measured quality gain. |
| **Skills** | Versioned playbooks delivered into the session and followed verbatim. | Attachment is a silent no-op without a prompt paragraph naming the path (3/3); no listing tool, no auto-selection by scope. |
| **Human interaction & steering** | Interrupt and redirect mid-flight (0.5 s, genuine re-planning); a custom tool parks a session on a human — 75 min parked, 0.2 s resume, ~free. | Ergonomics are yours: no message queueing (bare `user.message` is HTTP 400), and `requires_action` can't distinguish human-wait from machine-wait or stranded. |
| **Integrations** | MCP servers pinned per agent version with vault-held secrets; a session took a PR red→green in 10 MCP calls / $58. Remote tools survive worker death. |Depth == the MCP server's depth: no native PR/CI/review model, no per-user identity (everything is the vault PAT), browser/computer use rejected outright, and no external trigger. |
| **Configuration & release** | Agents fully as code: nine fields, immutable versions, 409 concurrency, pinning fails closed; 12 agents in 4.8 s; canarying free. | Definition store, not release system: no aliases/channels, no version delete/prune, no server-side desired state, and no deterministic replay. |
| **Scheduling & automation** | The only native wake: cron deployments with full session capability; 8/8 punctual fires, ~61 s to notice a new Linear issue. | Stateless trigger, 1-min floor: every fire is a cold session with no dedupe (3 overlaps), no retry, no catch-up; one bad dependency silently pauses everything. |
| **Observability** | Exactly replayable event log, 9 ms SSE, cost reconciling to the cent across threads, 164-session rollup in 0.96 s; hard budget ceilings. | Forensics, not operations: no aggregates, error rates, alerting or stuck-session signal; history and webhooks are disjoint; budgets overshoot 2.2×; attribution is your own metadata discipline. |
| **Reliability & recovery** | Fenced leases, native lease-expiry detection (~360 s), transient retries, and re-attach with nothing but the environment key. | Self-diagnosing, never self-healing: nothing re-dispatches (`work_id == session_id`), billing death is inert, and recovery re-runs unresolved tool calls — idempotency is on you. |
| **Model selection / routing** | Model as code: per agent version, per subagent, plus one advisor model. | Choice, not routing: pinned at creation and immutable, so no per-step routing, fallback or cost-based downgrade — and the choice silently changes capability class (Haiku never compacts). |
| **Execution environments** | A private execution plane: outbound-only workers, per-session sandbox, persistent volume workspace, memory mount, heartbeats and fencing. | No fleet layer: the queue has zero routing (a worker claimed 4 sibling sessions), one Console-only all-powerful env key, and no snapshot/known-good re-entry — only the volume survives. |
