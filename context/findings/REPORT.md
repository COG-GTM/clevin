...
|---|---|---|
| **Autonomous execution** — sessions, agent loop, `bash`/file tools, compaction, prompt caching, `user.define_outcome` | A hosted, durable reasoning loop that plans, edits files and runs commands unattended for hours, compacts its own context, and can be graded against a declared outcome rubric. Measured: 12/13 graded runs 4/4 on a 27-file `float`→`Decimal` migration, 0 nudges across all 10 completed runs, $41–62, 2–6.5 min; a 5.4 MB / 60-turn Opus run compacted twice (867,646→40,753 tokens, 95.3%) with exact early-constraint recall; 93% cache-read fraction. | The work itself reaches parity; what's missing is any notion of a **job** wrapped around it. The platform's unit is a *turn*, not a *task*, with four consequences: (1) **stopping ≠ finishing** — a budget cap is enforced at event admission, so runs die at an arbitrary turn boundary (one stopped mid-edit, J's stopped one tool call before `git push`) and nothing marks the work incomplete; (2) **the stop reason says nothing about the objective** — tool crash, worker death and the model giving up all collapse into `retries_exhausted`, and there is no "failed because tests never passed"; (3) **no transactional boundary** — event history, the workspace volume and the Memory Store commit independently, so a run can end with memory asserting a fact the workspace doesn't reflect (exactly how J's poisoned memory entry arose); (4) **nothing owns the work once the loop stops** — no deadline, no retry policy, no re-dispatch (§4.1). Plus: compaction is a property of the model, not the platform (Haiku never compacts and dies at 200 K), and there is no verification primitive — testing exists only because bash does, the model's own success claim is untrustworthy (every arm claimed success; the SHA-protected grader failed one), and the native `user.define_outcome` rubric is model-judged, slow (>13 min under load) and used by 0 of 164 observed sessions. |
