...
### 3.1 Summary table — by capability bucket

Twelve buckets, each mapped to the native primitives behind it, what this program actually built
with it, and where it tops out against Devin's out-of-the-box behaviour.

| Capability bucket | Native primitives behind it | What you can build with it (measured) | Upper bound vs Devin out-of-the-box | Class |
|---|---|---|---|---|
| **Autonomous execution** | Sessions, agent loop, `bash`/file tools, compaction, prompt caching, `user.define_outcome` | Unattended multi-file coding: 12/13 graded runs 4/4 on a 27-file `float`→`Decimal` migration, 0 nudges in all 10 completed runs, $41–62, 2–6.5 min (B). Long context is real: 5.4 MB / 60-turn Opus run compacted twice, 867,646→40,753 tokens (95.3%), exact early-constraint recall (A); 93% cache-read fraction (I). Verification is bash — SHA-protected in-sandbox graders ran clean 13/13, no test-gaming (B); `user.define_outcome` graded a run `needs_revision`, catching `CPU count:` vs required `cpus:` (H-17) | No durable notion of a *job*: a budget stop is a mid-edit guillotine and every failure collapses into `retries_exhausted`. Compaction is model-dependent (Haiku never compacts, dies at 200 K). No testing primitive — verification exists only because bash does; the model's own success claim is untrustworthy (every B arm claimed success; the grader failed one). Outcome rubrics are slow (>13 min under load) and were used by 0 of 164 observed sessions | A |
| **Memory & learning** | Memory Stores (mount + API), CAS writes, provenance, redaction; Dreams (research preview, *untested here*) | Durable, versioned, attributable storage: 260 writes / 12 workers / 4.7 s with zero failures; CAS via `expected_content_sha256` (1 winner, 5×409); full provenance and redaction (E). Cross-session learning pays when rediscovery is expensive: interpreter-mismatch replay went from 3 install attempts / 69 s / $17 to 1 attempt / 45 s / $12 (K-13); J's agent detected and corrected a memory entry poisoned by its own subagent | **Storage, not retrieval**: nothing is ever injected — retrieval is the model choosing to `rg` a mount, under an Anthropic-authored memory prompt you can only append to. Wide (200-entry) and narrow (2-entry) stores cost the same because entries aren't loaded. Memory is a permanent injection surface (redact, don't rewrite history) and cheap facts cost more to remember than to rediscover (write-back round = 2.7× cold baseline, E-15). Dreams (async store dedup/synthesis from 1–100 transcripts) is the native curation answer but is access-gated behind `dreaming-2026-04-21` and untested by this program | A store / C retrieval |
| **Subagents / delegation** | `multiagent` coordinator roster; implicit `create_agent` / `send_to_agent` / `list_agents` / `send_to_parent`; `{type:"self"}`, `{type:"advisor"}` | ≥7 concurrent children with per-thread cost attribution, tool grants enforced at the tool layer, roster version pinning, evidence-based synthesis of deliberately conflicting child reports (F). Subagents are ordinary agent resources referenced by ID — the repo ships three (explorer, test-debugger, adversarial-reviewer) | Depth 1, enforced twice (a rostered agent may not have a roster; a child never receives `create_agent`). Zero runtime control: no cancel, timeout or heartbeat from inside; ~533 KB child replies silently vanish 3/3; concurrent child edits are silent last-writer-wins. 3–4× the cost of solo with **no measured quality gain** on either a visible- or hidden-oracle task | A |
| **Skills** | Skill resources attached to an agent version | Versioned playbooks the agent follows verbatim once told where they live; J's gauntlet followed its PR-hygiene Skill end-to-end (K-12, J-6) | Attachment alone is a **silent no-op** — 3/3 sessions denied the playbook existed until a system-prompt paragraph named the path. No listing/loading tool, no agent-side publishing, no auto-selection by scope: the "which playbook applies here" layer doesn't exist | C |
| **Human interaction & steering** | `user.interrupt`, `user.message`, event stream, custom tools + `requires_action` | Steering at parity: `user.interrupt` accepted in 0.5 s, producing genuine re-planning with a correct progress audit (vs 95 s waiting out a tool call without it) (K-1/2). Ask-and-block: a custom `ask_human` tool parks a session indefinitely — answered at 900 s and 4501 s, resumed in 0.2 s, $18 for 945 s wall (20.3 s active), so parking is ~free (K-4/5) | No "queue my message" primitive — a bare `user.message` to a working session is HTTP 400, so interrupt-then-message is client logic you write. No native ask-a-human state: `requires_action` is overloaded (machine-wait and human-wait look identical) and something you run must notice the park and answer it | A/B |
| **Integrations** | `mcp_servers` on the agent version; vaults + vault credentials | Real SCM/tracker work: one session took PR #6 red→green — read diff/comments/checks, fixed, tested locally, pushed, polled checks to `success`, replied inline in 10 MCP calls / 91 s active / $58 (K-9); Linear→GitHub end-to-end in J. MCP inverts reliability intuition: `agent.mcp_tool_use` is never dispatched locally, so remote tools are immune to worker death (C-14) | Integration depth == the MCP server's depth: no native PR, CI, review or repo model, and no user-level attribution (identity is the vault PAT, so every action is "the agent"). Browser/computer-use toolsets are rejected outright, so e2e work can run headless but can never be *shown*. The mechanics are at parity; the missing piece is the **trigger** (§4.3) | A / C |
| **Configuration & release** | Nine-field agent resource, immutable versions, 409 concurrency, per-version pinning | 100% code-expressible agents: drift == [] round-trips, byte-exact roll-forward rollback, pinning fails closed (404 on a bad version), 12 agents provisioned in 4.8 s (D). Canarying is free: three pinned versions over one benchmark with zero orchestration and per-variant cost from `session.usage` (D-12) | No aliases or channels ("prod" cannot be a pointer); no version delete/tag/prune; no server-side desired state, so convergence is client code (and 5–6 server-added fields per agent make naive diffing worthless — hence `drift.ts`); no reverse index from version→sessions; no deterministic replay (identical pin+prompt gave 164 vs 248 output tokens) | A/B |
| **Scheduling & automation** | Deployments (cron), lifecycle webhooks | The only native wake mechanism, and a capable one: full session capability per fire (subagents, MCP, memory, outcome rubric), 8/8 punctual fires with median 5.9 s jitter, ~61 s to notice a new Linear issue by polling (H, K) | Stateless cron with a one-minute floor: every fire is a **new cold session** with no dedupe (3 concurrent overlaps observed), no retry, no catch-up, no in-flight cap; one bad dependency silently pauses the whole deployment (an archived agent = invisible outage). No event ingress at all — webhooks/PR comments/Slack cannot start a session | A trigger / D lifecycle |
| **Observability** | Ordered event history, SSE, `session.usage`, metadata, budgets, `retry_status` | Best-in-class forensics: full reconstruction (version→span→tool→cost) from 1–2 API calls, cost reconciling to the cent across threads, SSE median 9 ms / max 18 ms, fleet rollup of 164 sessions / $104.42 in 2 calls / 0.96 s; `retry_status` is the recoverable-vs-fatal discriminator (I). Budgets are hard ceilings enforced at event admission (turn 12 rejected after 11 ran) | Forensics, not operations: no aggregate/error-rate/alerting API, no stuck-session signal, no changed-files record, no tool-call→effect join. History and webhooks are disjoint surfaces (5 of 35+44 types overlap; budget/`requires_action`/idled are webhook-only). Budgets overshoot (a single turn blew a $0.05 cap to $0.11, 2.2×) with no wrap-up turn. Attribution is your discipline: 50 sessions / $15.79 were permanently unattributable | A/C |
| **Reliability & recovery** | Work-item leases, heartbeats, `SessionToolRunner` re-attach, native retries | Correct primitives where they exist: leases are properly fenced (a resurrected worker is shut out), lease expiry is detected natively ~360 s after the last heartbeat, transient tool errors retry, and a session can be recovered by re-attaching a bare `SessionToolRunner` with nothing but the environment key (C) | **Self-diagnosing, never self-healing.** Nothing re-dispatches: `work_id == session_id`, so reclaiming polls re-offer nothing and a dead worker strands the session in `requires_action` forever; webhook replay spawns nothing; a billing-dead session stayed inert after credits were restored. No server-side tool deadline, and native recovery re-executes any tool call lacking a result — it cannot tell "never ran" from "ran, then the worker died", so idempotency is your problem (§4.1) | C / D |
| **Model selection / routing** | `model` on the agent version; per-roster-entry models; one `advisor` | Model choice as code: pin a model per agent version, give each subagent a different one, consult one advisor model — all decided before the session | **No routing.** A session is pinned to one model at creation and immutable for life (`model` isn't even a session-update field). No per-step routing, no fallback on failure, no cost-based downgrade, no "cheap model to explore, Opus to fix". And the choice silently changes capability class: Opus compacts at ~868 K, Haiku never compacts and dies at 200 K (§4.5) | A config / D routing |
| **Execution environments** | Environment resource, environment key, work queue, `SessionToolRunner`; your own sandbox (Modal here) | A genuinely private execution plane: outbound-only workers, one gVisor Modal sandbox per session named by session ID, persistent volume workspace, Memory Store mounted at `/mnt/memory`, heartbeat + split-brain fencing (C, architecture §2) | The queue has **zero routing** — any worker holding the env key can claim any session's work item (C accidentally claimed 4 sibling sessions), so hardware affinity, admission control and concurrency caps are yours to build. One all-powerful, Console-only environment key (no API minting, no per-engineer scoping, no user object anywhere). No snapshot/verify/known-good re-entry primitive: only the volume survives a sandbox's ~1 h life; tool edges are 100 KiB output / 120 s / unconfined bash | A plane / D fleet |
