...
|---|---|---|
| **Autonomous execution** | An agent that works on its own for hours: it plans, edits files, runs commands, and shortens its own history when it gets too long. 12 of 13 graded runs scored full marks on a 27-file change with no human help, at $41–62 each. | It runs turns, not jobs. Stopping is not the same as finishing: a spend cap ended one run in the middle of an edit. Every kind of failure is reported the same way, so you cannot tell why it stopped. And there is no built-in way to check the work — testing only happens because the agent can run shell commands. |
| **Session management** | A session is a long-lived, addressable piece of work. You can come back to it later and it still has its history, its sandbox, and its place in the repo. One resumed in 0.2 seconds after waiting 75 minutes. You can list every session in the account. | Once a session starts, almost nothing about it can change: you can swap its tools, but not its model, prompt, or instructions. You also cannot copy or branch a session, so you can never try two approaches from the same starting point. |
| **Memory & learning** | A shared store of notes that survives between sessions, mounted as files in the sandbox, with safe concurrent writes. 12 agents wrote 260 notes in 4.7 seconds. On a repeat task, memory cut the work from 69 s/$17 to 45 s/$12. | It stores knowledge but never retrieves it for you. Nothing is added to the agent's context automatically — the agent has to go read the files itself, so a big store is no more useful than a small one unless the agent looks. Old notes can only be hidden, never deleted. |
| **Subagents / delegation** | An agent can hand work to helper agents that run at the same time in their own contexts, each with its own cost and its own limited set of tools. We ran seven at once. | Only one level deep, and only to helpers listed in advance — an agent can never invent a new helper while running. The parent cannot cancel, time out, or check on a helper, very large replies are dropped silently, and helpers cost 3–4× more without producing better work in our tests. |
| **Skills** | Reusable written procedures you attach to an agent, which it then follows step by step. | Attaching one does nothing on its own: in 3 of 3 tests the agent did not know the procedure existed until the prompt told it where to look. There is no way for the agent to browse available procedures or pick the right one for the task. |
| **Human interaction & steering** | You can interrupt an agent mid-work and redirect it (accepted in 0.5 s, and it genuinely re-planned), and the agent can stop and wait for a person. Waiting is nearly free: one session waited 75 minutes and resumed instantly. | The convenience layer is missing. You cannot simply queue a message for a busy agent — sending one is rejected, so you must interrupt first. And a waiting agent looks identical to a stuck one, so you cannot tell whether it needs a person or is broken. |
| **Integrations** | The agent can use outside tools such as GitHub and Linear, with credentials kept in a managed vault and locked to a specific agent version. One session took a failing pull request to passing in 10 tool calls for $58. These calls keep working even if your own machine dies. | The agent only knows as much about GitHub as the connector exposes — there is no real understanding of pull requests, CI, or reviews, and every action looks like it came from one shared account rather than a person. It also cannot use a browser, so it can run tests but never show them. Nothing outside can start a session. |
| **Configuration & release** | The whole agent is defined in code: nine fields, versions that can never change once published, safe concurrent edits, and rollback by republishing an old version. Twelve agents set up in 4.8 seconds, and running two versions side by side costs nothing extra. | It stores definitions well but does not manage releases. There is no "production" label you can point at a version, no way to delete or tidy old versions, and no server-side record of what you intended — keeping things in sync is your code's job. Reruns are not repeatable either. |
| **Scheduling & automation** | The only built-in way for an agent to start without a person: scheduled runs, with everything a normal session can do. All 8 scheduled runs fired on time, and one noticed a new Linear ticket about a minute after it was filed. | It is a timer, not a work manager. The fastest schedule is once a minute, every run starts from scratch, and nothing prevents duplicates — we saw three runs going at once. Failed runs are never retried, and one broken setting quietly stops the whole schedule. |
| **Observability** | A complete, replayable record of everything an agent did, live streaming with 9 ms delay, exact costs down to the cent including helper agents, and a whole-account summary in under a second. Spend caps are enforced. | You can explain the past but not manage the present: no summaries, error rates, or alerts, and nothing tells you a session is stuck. Spend caps overshoot by roughly 2×, and cost only ties back to a team or ticket if you remember to tag every session yourself. |
| **Reliability & recovery** | Work is leased, so two machines can never fight over the same session, and an abandoned session is noticed automatically after about six minutes. A session can be picked up again with nothing but an environment key. | It notices problems but never fixes them. Nothing hands the work to another machine, so an abandoned session waits forever until a person acts. A session killed by running out of credit never restarts, even after paying. And recovery may re-run a command that already ran. |
| **Model selection / routing** | You choose the model in code — per agent, per helper agent, plus one model you can consult for advice. | You choose once and live with it: the model is fixed when the session starts, so there is no switching mid-task, no falling back when one fails, and no using a cheap model to explore and a strong one to fix. The choice also quietly changes what the agent can do — a small model cannot handle long tasks at all. |
| **Execution environments** | Your code runs on your own infrastructure while the thinking stays hosted: outbound-only connections, a fresh sandbox per session, a workspace that survives, and safeguards against two machines running the same work. | There is no fleet management. Work is handed to whichever machine asks first, so you cannot send certain jobs to certain machines or limit how many run at once — one worker accidentally picked up four other sessions' work. Access is a single all-powerful key created by hand, and there is no way to save and return to a known-good machine state. |
